<template>
  <v-layout>
    <v-toolbar fixed app>
      ...
      <v-icon large color="red" @click="$router.push({name:'home'})">fa-h-square</v-icon>
      ...
      <v-toolbar-items class="hidden-sm-and-down">
        ...
        <v-btn flat v-if="name != null" @click="logout" exact>
          <strong>Logout</strong>
        </v-btn>

        <v-btn flat router :to="{name:'portfolio'}" exact>
          <strong>Portfolio</strong>
        </v-btn>
        <v-btn flat router :to="{name:'post'}" exact>
          <strong>Post</strong>
        </v-btn>
        <v-btn flat router :to="{name:'translate'}" exact>
      ...
      <v-btn class="hidden-sm-and-down" fab small @click="alertBookmark">
        <v-icon size="25px" color="yellow darken-1">fa-star</v-icon>
      </v-btn>
    ...
    <v-navigation-drawer v-model="drawer" fixed temporary right>
      <v-toolbar flat>
        ...
        <v-btn fab small @click="alertBookmark">
          <v-icon size="25px" color="yellow darken-1">fa-star</v-icon>
        </v-btn>
      </v-toolbar>
  ...
  methods: {
    ...
    async signUp() {
      if (this.cname != null && this.cemail != null && this.cpassword != null) {
        const result = await FirebaseService.registerWithEmail(
          this.cemail,
          this.cpassword
        );
